...
# # 调整整体亮度
# img = modify_brightness(img, factor=-10)
# # 调整曝光度
...
# 阴影调整（factor正数增量，负数降暗）（luminance_threshold越高则将更大的部分划归为阴影，更小的部分划分为高光）
cvimg = pilImage_to_cvarray(img)
cvimg = modify_shadow(cvimg, factor=90, luminance_threshold=20)
img = cvarray_to_pilImage(cvimg)
# 高光调整（factor正数增量，负数降暗）（luminance_threshold越高则将更大的部分划归为阴影，更小的部分划分为高光）
cvimg = pilImage_to_cvarray(img)
cvimg = modify_highlight(cvimg, factor=-40, luminance_threshold=90)
img = cvarray_to_pilImage(cvimg)
# # 调整色温（负蓝正黄）
# cvimg = pilImage_to_cvarray(img)
